...
test('composes prompt layers in the expected order', async () => {
  ...
  assert.deepEqual(composition.layers.map((layer) => layer.key), ['base', 'role', 'memory', 'extensions', 'taskContext', 'taskInstruction']);
  ...
  assert.ok(baseIndex < roleIndex && roleIndex < memoryIndex && memoryIndex < extensionsIndex && extensionsIndex < taskContextIndex && taskContextIndex < taskIndex);
